...
def my_perceptron(x):
    ...
    w = tf.get_variable('w',shape = [x] ,dtype = tf.float32, initializer=tf.ones_initializer(),trainable=True)
    mul = tf.multiply(i,w)
    summ = tf.reduce_sum(mul)
    out = my_relu(summ)
...
def onelayer(X, Y, layersize=10):
    ...
    """
    ww = tf.zeros([784, layersize])
    w = tf.Variable(ww)
    bb = tf.zeros([layersize])
    b = tf.Variable(bb)
    prelogits = tf.matmul(X,w)
    logits = prelogits + b
...
def twolayer(X, Y, hiddensize=30, outputsize=10):
    ...
    prelog1 = tf.matmul(X, w1)
    logits1 = prelog1 + b1
    preds1 = tf.nn.relu(logits1)
    prelog = tf.matmul(preds1, w2)
    logits =  prelog + b2
    preds = tf.nn.softmax(logits)
    batch_xentropy = tf.nn.softmax_cross_entropy_with_logits(labels=Y, logits=logits)
    batch_loss = tf.reduce_mean(batch_xentropy)
...
def convnet(X, Y, convlayer_sizes=[10, 10], \
    ...
    """
    # this is  convnet 1 layer
    conv_shape1 = [filter_shape[0], filter_shape[1], 1, convlayer_sizes[0]]
    #a1 = tf.truncated_normal(conv_shape1, stddev=0.1)
    convnet_1_w = tf.Variable(tf.truncated_normal(conv_shape1, stddev=0.1))
    #a2 = tf.constant(0.1, shape=[convlayer_sizes[0]])
    convnet_1_b = tf.Variable(tf.constant(0.1, shape=[convlayer_sizes[0]]))
    #a3 = tf.nn.conv2d(X, convnet_1_w, strides=[1, 1, 1, 1], padding='SAME')
    conv1 = tf.nn.relu(tf.nn.conv2d(X, convnet_1_w, strides=[1, 1, 1, 1], padding='SAME') + convnet_1_b)
    # this is convnet 2 layer
    conv_shape2 = [filter_shape[0], filter_shape[1], convlayer_sizes[0], convlayer_sizes[1]]
    #b1 = tf.truncated_normal(conv_shape2, stddev=0.1)
    #print (b1)
    convnet_2_w = tf.Variable(tf.truncated_normal(conv_shape2, stddev=0.1))
    #b2 = tf.constant(0.1, shape=[convlayer_sizes[1]])
    #print (b2)
    convnet_2_b = tf.Variable(tf.constant(0.1, shape=[convlayer_sizes[1]]))
    #b3 = tf.nn.conv2d(conv1, convnet_2_w, strides=[1, 1, 1, 1], padding='SAME')
    #print (b3)
    conv2 = tf.nn.relu(tf.nn.conv2d(conv1, convnet_2_w, strides=[1, 1, 1, 1], padding='SAME') + convnet_2_b)
    #compute x_flat

    xsize = 28 * 28 * convlayer_sizes[1]
    #print (xsize)
    X_flat = tf.reshape(conv2, [-1, xsize])
    # get w and b
    w = tf.Variable(tf.zeros([xsize, outputsize]))
