...
class ReplayBuffer(object):
    """Record Experiences and randomly replay"""
...
class DoubleQNetwork(object):
    ...
    def _network_template(self, state_ph):
        """
        Since we use double deep Q learning, we use this function as network template
        for both target network and online network MLP network defines here
        """
        h1 = tf.layers.dense(inputs=state_ph,
                units=self.n_h1,     
                activation=tf.nn.tanh,          
                kernel_initializer=tf.random_normal_initializer(seed=123),
                use_bias=True,                  
                name='h1')                      
       ...
       #         use_bias=True,                  
       #         name='h2')                      
        outputs = tf.layers.dense(inputs=h1,
                activation=None,                
                units=self.n_output,          
                use_bias=True,                  
                kernel_initializer=tf.random_normal_initializer(seed=123),
    ...
    def sync_nets_op(self):
        ...
        for (var_eval, var_target) in zip(vars_in_eval_qnet, vars_in_target_qnet):
            sync_ops.append(var_target.assign(var_eval * self.update_rate + var_target * (1 - self.update_rate), use_locking=True))
        return sync_ops
...
class Agent(object):
    ...
    def __init__(self, params):
        self.params = params
        self.replay_buffer = ReplayBuffer(params['memory_size'])
        self.double_q_nets = DoubleQNetwork(params['input_dim'], params['output_dim'], params['update_rate'])
    ...
    def is_train_stop(self, cur_episode):
        # when exceeds maximum episode limitation
        if cur_episode > self.params['max_training_episode']:
            return True
        # at least training rounds is needed
        if self.n_training_steps < self.params['min_training_steps']:
            return False
        # latest reward is not enough
    ...
    def build_feed_data(self, minibatch_exp):
        ...
        return dict({self.params['target_ph']: target, self.params['action_ph']: action, self.params['state_ph']:state})
    ...
    def train(self, cur_episode):
        if self.train_stop_flag == True:
            return
        
        if self.is_train_stop(cur_episode) == True:
